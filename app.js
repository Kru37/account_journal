...
const UiCntrl = (function () {
  ...
  return {
    populateTable: function (items) {
      //   document.querySelector(uiselector.table_end).style.display = "block";
      let html = "";
      const end_table = document.querySelector(uiselector.table_end);
      items.forEach((item) => {
        ...
        document
          ...
          .insertBefore(tr, end_table);
      });
      //   html += ` <tr class="table_end">
      //         <td></td>
      //         <td>Total</td>
      //         <td></td>
      //         <td class="debit_cost">${debit}</td>
      //         <td class="credit_cost">${credit}</td>
      //       </tr>`;
    ...
    clearUI: function () {
      ...
      tr_list.forEach((item) => {
        if (item.classList.contains("table_end")) {
          item_add = item;
        }
      });

      document.querySelector(uiselector.tablebody).innerHTML = "";
      document.querySelector(uiselector.tablebody).appendChild(item_add);
    },
...
const App = (function (ItemCntrl, UiCntrl, StorageCntrl) {
  const loadEventlistener = () => {
    ...
    document
      .querySelector(selector.update)
      .addEventListener("click", handleUpdate);
    document
      .querySelector(selector.deletebtn)
      .addEventListener("click", hanldeDelete);
    document
      .querySelector(selector.back)
      .addEventListener("click", UiCntrl.cleareditstate);
  };
  const itemAdd = (e) => {
    const input = UiCntrl.getItemInput();
    if (input.title !== "" && input.amount !== "") {
      const newitem = ItemCntrl.addnewItem(
        input.date,
        input.title,
        input.ref,
        input.credit,
  ...
  const handleUpdate = (e) => {
    const input = UiCntrl.getItemInput();
    const updatedItem = ItemCntrl.updateDataItem(
      input.title,
      input.credit,
      input.debit,
      input.ref
